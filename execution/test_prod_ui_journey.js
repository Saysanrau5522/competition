...
async function testProdUiJourney() {
  console.log('🚀 Testing Live Production PDF Generation on https://competition.saysanrau.workers.dev...');
  ...
  try {
    ...
    const downloadDir = path.resolve(__dirname, '../.tmp/downloads_prod');
    ...
    console.log('1. Navigating to https://competition.saysanrau.workers.dev...');
    await page.goto('https://competition.saysanrau.workers.dev', { waitUntil: 'networkidle2' });

    console.log('2. Step 1: Submitting enterprise demographics...');
    await page.type('#companyName', 'Bina Jaya Construction Sdn Bhd');
    await page.select('#industrySector', 'construction_engineering');
    ...
    await page.waitForSelector('.dynamic-option-card', { visible: true, timeout: 15000 });
    ...
    await page.type('#contactName', 'Dato Ir. Jeffrey Tan');
    await page.type('#contactPhone', '+60193456789');
    await page.type('#contactEmail', 'jeffrey.tan@binajaya.my');
    await page.click('#btnSubmitAudit');

    console.log('7. Waiting for results section to be visible...');
    await page.waitForSelector('#resultsSection', { visible: true, timeout: 20000 });
    ...
    if (stats.size > 100000 && pageMatches.length === 4) {
      console.log('🎉 SUCCESS: Live production download generated a verified 4-page PDF with zero blank pages!');
    } else {
...
testProdUiJourney().catch(err => {
  console.error('❌ E2E Production Error:', err);
  process.exit(1);
});
